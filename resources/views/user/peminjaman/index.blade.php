...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('keydown', function(e) {
        ...
        // They usually end with Enter key
        if (e.key === 'Enter') {
            if (lastScanned) {
            ...
            // Reset accumulated input after 100ms of no input
            clearTimeout(lastScannedTimeout);
    ...
    document.querySelector('button[type="button"]:last-child').addEventListener('click', function() {
        const assignmentLetter = document.querySelector('#simple-search').value;
        
        if (!assignmentLetter) {
            showAlert('error', 'Please enter assignment letter number');
            return;
        }
        
        fetch('/peminjaman/save', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            },
            body: JSON.stringify({ assignment_letter: assignmentLetter })
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
        ...
        .catch(error => {
            showAlert('error', 'Error saving peminjaman');
            console.error('Error:', error);
        });
    });
});
...
function removeItem(itemId) {
    fetch(`/peminjaman/remove/${itemId}`, {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        }
